...
from qdax.core.neuroevolution.networks.maddpg_networks import make_maddpg_networks
from qdax.core.neuroevolution.mdp_utils import TrainingState, get_first_episode
from qdax.core.neuroevolution.losses.maddpg_loss import (
    maddpg_critic_loss_fn,
    maddpg_policy_loss_fn,
    maddpg_policy_loss_fn_v2
)
...
class MADDPGTrainingState(TrainingState):
...
class MADDPG:
    ...
    Gradient agent (DDPG), ref: https://arxiv.org/pdf/1802.09477.pdf
    """

    def __init__(self, config: MADDPGConfig, action_sizes: dict[int, int]):
        self._config = config

        self._policy, self._critic, = make_maddpg_networks(
    ...
    def init(
        ...
        """Initialise the training state of the DDPG algorithm, through creation
        ...
        training_state = MADDPGTrainingState(
    ...
    def select_action(
        ...
        """Selects an action according to DDPG policy. The action can be deterministic
    ...
    def play_step_fn(
        ...
        deterministic: bool = False,
    ) -> Tuple[EnvState, MADDPGTrainingState, Transition]:
        """Plays a step in the environment. Selects an action according to DDPG rule and
    ...
    def play_qd_step_fn(
        ...
        env: MultiAgentBraxWrapper,
        deterministic: bool = False,
    ) -> Tuple[EnvState, MADDPGTrainingState, QDTransition]:
        """Plays a step in the environment. Selects an action according to DDPG rule and
        performs the environment step.

        Args:
            env_state: the current environment state
            training_state: the DDPG training state
            env: the environment
    ...
    def eval_policy_fn(
        ...
        Args:
            training_state: DDPG training state.
    ...
    # Update the MADDPG.update method to use the new approach
    def update(
        self,
        training_state: MADDPGTrainingState,
    ...
    ) -> Tuple[MADDPGTrainingState, ReplayBuffer, Metrics]:
        ...
        # Update Critic
        random_key, subkey = jax.random.split(random_key)
        critic_loss, critic_gradient = jax.value_and_grad(maddpg_critic_loss_fn)(
        ...
        # Update policy
        policy_losses, policy_gradients = maddpg_policy_loss_fn(
            policy_params=training_state.policy_params,
        ...
        policy_params, target_policy_params, policy_optimizer_state = update_policy_step()
