...
The same as ma_pqg_emitter_v2.py. The loss is now a method of the class,
...
from qdax.core.neuroevolution.buffers.evorl_buffer import ReplayBuffer, ReplayBufferState
...
class QualityMAPGEmitter(Emitter):
    ...
    def init(
        ...
        dummy_transition = jax.tree_util.tree_map(
            lambda x: jnp.squeeze(x), dummy_transition
        )
        replay_buffer_state = self._replay_buffer.init(spec=dummy_transition)
        ...
        replay_buffer_state, _, _ = self.warmstart_buffer(
        ...
        print(f"Complete warmstart current replay buffer size: {replay_buffer_state.buffer_size}")
    ...
    def warmstart_buffer(
        ...
        env_state: EnvState,
        env_batch_size: int,
    ) -> Tuple[ReplayBufferState, EnvState, RNGKey]:
        ...
        transitions = jax.tree_util.tree_map(
            lambda x: jax.lax.collapse(x, 0, 2), transitions
        )
        replay_buffer_state = self._replay_buffer.add(replay_buffer_state, transitions)

        return replay_buffer_state, env_state, random_key
    ...
    def _apply_safe_mutation_to_subset(
        ...
        transitions = self._replay_buffer.sample(
            emitter_state.replay_buffer_state, subkey
        )
    ...
    def state_update(
        ...
        replay_buffer_state = self._replay_buffer.add(emitter_state.replay_buffer_state, transitions)
        # add transitions in the replay buffer

        emitter_state = emitter_state.replace(replay_buffer_state=replay_buffer_state)
    ...
    def _train_critics(
        ...
        replay_buffer_state = emitter_state.replay_buffer_state
        random_key, subkey = jax.random.split(random_key)
        transitions = self._replay_buffer.sample(
            replay_buffer_state, subkey
        )
