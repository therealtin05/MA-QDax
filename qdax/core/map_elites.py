...
from qdax.core.containers.mapelites_repertoire import MapElitesRepertoire, MultiAgentMapElitesRepertoire
...
class MultiAgentMAPElites:
    ...
    def __init__(
        ...
        emitter: Emitter,
        metrics_function: Callable[[MultiAgentMapElitesRepertoire], Metrics],
        qd_offset: float = 0.0,
    ) -> None:
        self._scoring_function = scoring_function
        self._emitter = emitter
    ...
    def init(
        ...
        centroids: Centroid,
        random_key: RNGKey,
    ) -> Tuple[MultiAgentMapElitesRepertoire, Optional[EmitterState], RNGKey]:
        """
        Initialize a Map-Elites repertoire with an initial population of genotypes.
        Requires the definition of centroids that can be computed with any method
        such as CVT or Euclidean mapping.
        ...
        # init the repertoire
        repertoire = MultiAgentMapElitesRepertoire.init(
            genotypes=init_genotypes,
            fitnesses=fitnesses,
            descriptors=descriptors,
            centroids=centroids,
    ...
    def update(
        self,
        repertoire: MultiAgentMapElitesRepertoire,
        emitter_state: Optional[EmitterState],
        random_key: RNGKey,
    ) -> Tuple[MultiAgentMapElitesRepertoire, Optional[EmitterState], Metrics, RNGKey]:
        """
        Performs one iteration of the MAP-Elites algorithm.
        1. A batch of genotypes is sampled in the repertoire and the genotypes
            are copied.
    ...
    def scan_update(
        self,
        carry: Tuple[MultiAgentMapElitesRepertoire, Optional[EmitterState], RNGKey],
        unused: Any,
    ) -> Tuple[Tuple[MultiAgentMapElitesRepertoire, Optional[EmitterState], RNGKey], Metrics]:
        """Rewrites the update function in a way that makes it compatible with the
        jax.lax.scan primitive.
